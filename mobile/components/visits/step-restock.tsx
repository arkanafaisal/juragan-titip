...
interface SuggestedProductsUIProps {
  suggestedProducts: ProductLight[];
  handleAddRestock: (p: ProductLight) => void;
}

function SuggestedProductsUI({ suggestedProducts, handleAddRestock }: SuggestedProductsUIProps) {
...
interface RestockItemUIProps {
  item: any;
  index: number;
  control: any;
  handleRemoveRestock: (productId: number) => void;
  handleRestockQuantity: (index: number, qty: number, maxStock: number) => void;
}

function RestockItemUI({
  item,
  index,
  control,
  handleRemoveRestock,
  ...
  return (
    <Card className="flex-row items-center p-3 gap-3">
      <View className="flex-1">
        ...
        <Text className="text-primary font-bold text-caption mt-1">{formatRupiah(item.wholesalePrice)} /pcs</Text>
      </View>
