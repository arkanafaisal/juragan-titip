...
export function StepCheckout({ 
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-lg pb-xl">
      <div className="grid grid-cols-1  gap-md">
        ...
        <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden flex flex-col h-full">
          <div className="bg-surface-container-low px-md py-sm border-b border-outline-variant">
            <h3 className="font-body  text-body  font-bold flex items-center gap-xs text-text-primary">
              <ShoppingCart className="w-4 h-4   text-primary"/> TAGIHAN (Laku + Restock)
            </h3>
          </div>
          <div className="p-md flex-1 flex flex-col">
            ...
            <div className="mt-lg border-t-[1.5px] border-text-primary pt-md space-y-xs">
              ...
              <div className="flex justify-between font-h3  text-h3  font-bold mt-sm bg-primary/10 p-sm rounded-lg text-primary items-center">
                <span>TOTAL TAGIHAN:</span>
                <span className="font-data-md  text-data-md ">{formatCurrency(totalBilled)}</span>
              </div>
                  ...
                  <div className="pt-sm">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-3 py-2  bg-surface border border-outline text-text-primary font-data-md  font-bold focus:border-primary focus:ring-1 focus:ring-primary rounded-xl outline-none transition-all shadow-sm"
                      />
                    </div>
                  </div>

                  {/* SISA HUTANG DINAMIS */}
                  <div className={`flex justify-between font-h3  text-h3  font-bold mt-sm p-sm rounded-lg items-center transition-colors ${
        ...
        <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden flex flex-col h-full">
          <div className="bg-surface-container-low px-md py-sm border-b border-outline-variant">
            <h3 className="font-body  text-body  font-bold flex items-center gap-xs text-text-primary">
              <Package className="w-4 h-4   text-success"/> BARANG DI TOKO (Setelah Visit)
            </h3>
          </div>
          <div className="p-md flex-1 flex flex-col">
            <div className="space-y-sm flex-1">
                ...
                displayStockItems.map(item => {
                  ...
                  return (
                    <div key={item.productId} className="flex justify-between items-center border-b border-dashed border-outline-variant pb-sm last:border-0 last:pb-0">
                      ...
                      <p className={`font-data-md  text-data-md  font-bold ${item.total > 0 ? 'text-success' : 'text-text-muted'}`}>
                        {item.total}
                      </p>
                    </div>
                  );
            ...
            <div className="mt-lg border-t-[1.5px] border-outline-variant pt-md flex justify-between items-center bg-surface-bright -mx-md -mb-md p-md">
              <span className="font-body  text-body  font-bold text-text-secondary">Total Seluruh Item Aktif:</span>
              <span className="font-h3  text-h3  font-bold text-text-primary bg-surface-container px-3 py-1 rounded-md">
      ...
      {/* AKSI TOMBOL */}
      <div className="flex flex-col  gap-sm  justify-between items-center pt-md">
        <button onClick={onPrev} className="w-full  text-text-secondary hover:text-text-primary px-md py-sm  rounded-lg font-body  text-body  font-medium transition-colors border border-outline-variant hover:bg-surface-container-low bg-surface text-center">
          Kembali Edit
        </button>
        <div className="w-full  max-w-[400px] flex flex-col items-center  gap-1">
          {isNextDisabled && (
            <span className="text-error font-caption text-caption text-center ">
          ...
          <button 
            ...
            className="w-full bg-primary text-on-primary font-body  text-body  py-sm  rounded-xl font-bold flex items-center justify-center gap-sm hover:bg-primary/90 transition-all shadow-md active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100"
          >
